...
def alexa_intents_root():
    ...
    if ha_service and player_id:
        ...
        try:
            ...
            print(f"HA API: {ha_service} -> {r.status_code}", flush=True)
...
def status():
    ...
    try:
        ...
        if resp.ok:
            api_html = f'<span class="led green"></span> API reachable ({resp.status_code})'
        else:
            api_html = f'<span class="led red"></span> API responded {resp.status_code}'
    except RequestException as e:
        api_html = f'<span class="led red"></span> Error: {str(e)}'
    html = f"""<!doctype html><html><head><meta charset="utf-8"><title>Status</title>
    <style>body{{font-family:Arial,sans-serif;padding:20px}}.led{{display:inline-block;width:14px;height:14px;border-radius:50%;margin-right:8px}}.green{{background:#2ecc71}}.red{{background:#e74c3c}}.row{{margin:8px 0}}.muted{{color:#666;font-size:.9em}}</style>
    </head><body><h1>Service Status</h1>
    <div class="row">{skill_html}</div>
    <div class="row">{api_html}</div>
    <hr><div class="muted">Checked: {endpoint}</div></body></html>"""
